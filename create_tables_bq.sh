...
# Create Missing Tables Script using bq command
echo "Starting to create missing tables in dataset agri_data"

# Project and dataset settings
PROJECT_ID="agri-data-454414"
DATASET_ID="agri_data" 
SEED_DATASET="agri_data_seeds"
...
FROM 
    \`${PROJECT_ID}.${SEED_DATASET}.seed_farm\` sf
LEFT JOIN 
    \`${PROJECT_ID}.${SEED_DATASET}.seed_soil\` ss ON sf.farm_id = ss.farm_id
"

if [ $? -eq 0 ]; then
    echo "✅ farm_analysis table created successfully"
else
    echo "❌ Error creating farm_analysis table"
    exit 1
fi
...
FROM 
    \`${PROJECT_ID}.${SEED_DATASET}.seed_crop\` sc
LEFT JOIN 
    \`${PROJECT_ID}.${SEED_DATASET}.seed_harvest\` sh ON sc.crop_id = sh.crop_id
GROUP BY 
    sc.crop_type,
    sc.crop_name,
    sc.growing_season
...
if [ $? -eq 0 ]; then
    ...
    echo "❌ Error creating crop_analysis table"
    exit 1
fi
...
WITH synthetic_dates AS (
    SELECT 
        ...
        END AS harvest_date
    FROM \`${PROJECT_ID}.${SEED_DATASET}.seed_yield\` sy
    JOIN \`${PROJECT_ID}.${SEED_DATASET}.seed_crop\` sc ON sy.crop_id = sc.crop_id
)
SELECT
    sf.farm_id AS Farm_ID,
...
JOIN 
    \`${PROJECT_ID}.${SEED_DATASET}.seed_crop\` sc ON d.crop_id = sc.crop_id
LEFT JOIN 
    \`${PROJECT_ID}.${SEED_DATASET}.seed_harvest\` sh ON sf.farm_id = sh.farm_id AND sc.crop_id = sh.crop_id
LEFT JOIN 
    \`${PROJECT_ID}.${SEED_DATASET}.seed_yield\` sy ON sh.harvest_id = sy.harvest_id
LEFT JOIN 
    \`${PROJECT_ID}.${SEED_DATASET}.seed_sustainability\` sus ON sf.farm_id = sus.farm_id
LEFT JOIN 
    \`${PROJECT_ID}.${SEED_DATASET}.seed_soil\` ss ON sf.farm_id = ss.farm_id
"
...
if [ $? -eq 0 ]; then
    ...
    echo "❌ Error creating full_data table"
    exit 1
fi
