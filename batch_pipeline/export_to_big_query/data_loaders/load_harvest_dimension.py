...
def load_from_google_cloud_storage():
    """
    Function to load harvest dimension data from Google Cloud Storage.
    
    Returns:
        pandas.DataFrame: DataFrame containing harvest dimension data
    """
    # Set up credentials
    ...
    object_prefix = 'transformed/agri_data/harvest_dimension'
    
    # List all objects with the given prefix
    bucket = storage_client.bucket(bucket_name)
    blobs = list(bucket.list_blobs(prefix=object_prefix))
    
    print(f"Found {len(blobs)} files for harvest dimension.")
    ...
    if dfs:
        ...
        print(f"Loaded {len(concatenated_df)} harvest dimension records.")
        return concatenated_df
    else:
        print("No data loaded for harvest dimension.")
        return pd.DataFrame()
