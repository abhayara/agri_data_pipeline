...
def load_from_google_cloud_storage():
    """
    Function to load soil dimension data from Google Cloud Storage.
    
    Returns:
        pandas.DataFrame: DataFrame containing soil dimension data
    """
    # Set up credentials
    ...
    object_prefix = 'transformed/agri_data/soil_dimension'
    
    # List all objects with the given prefix
    bucket = storage_client.bucket(bucket_name)
    blobs = list(bucket.list_blobs(prefix=object_prefix))
    
    print(f"Found {len(blobs)} files for soil dimension.")
    ...
    if dfs:
        ...
        print(f"Loaded {len(concatenated_df)} soil dimension records.")
        return concatenated_df
    else:
        print("No data loaded for soil dimension.")
        return pd.DataFrame()
