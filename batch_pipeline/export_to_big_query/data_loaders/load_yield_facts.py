...
def load_from_google_cloud_storage():
    """
    Function to load yield facts data from Google Cloud Storage.
    
    Returns:
        pandas.DataFrame: DataFrame containing yield facts data
    """
    # Set up credentials
    ...
    object_prefix = 'transformed/agri_data/yield_facts'
    
    # List all objects with the given prefix
    bucket = storage_client.bucket(bucket_name)
    blobs = list(bucket.list_blobs(prefix=object_prefix))
    
    print(f"Found {len(blobs)} files for yield facts.")
    ...
    if dfs:
        ...
        print(f"Loaded {len(concatenated_df)} yield facts records.")
        return concatenated_df
    else:
        print("No data loaded for yield facts.")
        return pd.DataFrame()
